...
template<typename T> T ToUpperFunc(T Src, std::locale *Loc)
{
     return std::toupper(Src, *Loc);
}

template<class T>
     void MakeUpper(T &Src, const char *Lang = ".1250")
{
     static T::value_type (*pFunc)(T::value_type, std::locale*) = ToUpperFunc<T::value_type>;
